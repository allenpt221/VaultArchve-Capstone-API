...
import { CircleCheck, CircleX, Loader2, Search, SparklesIcon } from 'lucide-react';
...
function AIrecommendation() {
  ...
  const isValidationError =
    message.includes("random or meaningless") ||
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-12">
      ...
      <form onSubmit={handleSelect}>
        <Card className="border-border mb-10">
          ...
          <CardContent className="space-y-4">
            <Select value={selectedInterest} onValueChange={(value) => setSelectedInterest(value)}>
            ...
            <Textarea
              ...
              rows={3}
              className={`text-sm ${inlineError ? "border-red-400 focus-visible:ring-red-400" : ""}`}
            />
            ...
            {!inlineError && result.length > 0 && result[0]?.error ? (
              ...
              !inlineError && result.map((item, index) => (
      ...
      {alert && message && !isValidationError && (
        <Alert
          variant="default"
        ...
        >
          <div className="flex items-start gap-3">
            ...
            <div>
              <AlertTitle
                className={`${
                  isSuccess ? "text-green-800" : isLoading ? "text-blue-800" : "text-red-800"
